...
public class BottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public BottomSheetBehavior (Context context, AttributeSet attrs ) {
        ...
        a = context.obtainStyledAttributes(attrs, R.styleable.NearbyBottomSheetBehavior);
        if (attrs != null)
            mAnchorPoint = (int) a.getDimension( R.styleable.NearbyBottomSheetBehavior_anchorPoint, 0);
        a.recycle();
...
//              }
    ...
    public boolean onTouchEvent(CoordinatorLayout parent, V child, MotionEvent event ) {
        ...
        if ( mViewDragHelper == null ) {
            Log.e(TAG, "onTouchEvent: mViewDragHelper init --------------" );
            mViewDragHelper = ViewDragHelper.create( parent, mDragCallback );
        }
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, V child, View target, int dx, int dy, int[] consumed ) {
        ...
        if (( mLastStableState == STATE_COLLAPSED &&  newTop < mAnchorPoint )
...
//            mCallback.onStateChanged(bottomSheet, state);
    ...
    private final ViewDragHelper.Callback mDragCallback = new ViewDragHelper.Callback() {
        ...
        public void onViewReleased( View releasedChild, float xvel, float yvel ) {
            ...
            if ( yvel < 0 ) { // Moving up
                if (releasedChild.getTop() >= mAnchorPoint) {
                    top = mAnchorPoint;
                    targetState = STATE_ANCHOR_POINT;
                }else{
                    top = mMinOffset;
